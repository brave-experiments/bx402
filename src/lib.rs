...
mod tests {
    ...
    async fn upstream_5xx_is_relayed_byte_for_byte() {
        ...
        // Settle runs concurrently with the search, so a verified payment is charged
        // even when the upstream then 5xxes; the receipt rides back with the relayed
        // error so the client has proof of the payment it made.
        assert!(
            response.headers().contains_key("payment-response"),
            "a settled payment carries its receipt even on a relayed upstream error",
        );
        let bytes = response.into_body().collect().await.unwrap().to_bytes();
